...
(contextInitialized 方法将委托给 DmnEngines.init()。 这将寻找类路径上的 +flowable.dmn.cfg.xml+ 资源文件，并对于给定的配置创建一个 +DmnEngine+ (例如，带有配置文件的多个JAR).如果类路径上有多个此类资源文件，请确保它们都具有不同的名称。 当需要DMN引擎时，可以使用以下方式获取它：)
...
(context-listener的 contextDestroyed方法委托给 DmnEngines.destroy()。 这将适当地关闭所有初始化的DMN引擎。)